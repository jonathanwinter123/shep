...
- `src-tauri/target/release/bundle/macos/shep.app.tar.gz` (updater artifact)
- `src-tauri/target/release/bundle/macos/shep.app.tar.gz.sig` (updater signature)
- `latest.json` (updater metadata, generated by script)
...
gh release create vX.Y.Z \
  "src-tauri/target/release/bundle/dmg/shep_X.Y.Z_aarch64.dmg" \
  "src-tauri/target/release/bundle/macos/shep.app.tar.gz" \
  "src-tauri/target/release/bundle/macos/shep.app.tar.gz.sig" \
  "latest.json" \
  --title "Shep vX.Y.Z" \
  --notes-file - <<'EOF'
## Shep vX.Y.Z
...
The in-app updater verifies downloaded updates using an Ed25519 keypair. This
is separate from Apple's code signing.
...
```bash
pnpm tauri signer generate -w ~/.tauri/shep.key
```

This prints the **public key** to stdout — it's already configured in
`src-tauri/tauri.conf.json` under `plugins.updater.pubkey`.

The **private key** is saved to `~/.tauri/shep.key`. Keep it safe and never
commit it.

### Build-Time Usage

Set these env vars before `pnpm tauri build`:
...
With these set, the build automatically produces `shep.app.tar.gz` and
`shep.app.tar.gz.sig` alongside the DMG.

After building, run `bash scripts/generate-update-json.sh` to create the
`latest.json` metadata file, then include all three in `gh release create`.
